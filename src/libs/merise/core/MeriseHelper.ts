...
export default class MeriseHelper implements MeriseHelperInterface {
  private static instance: MeriseHelper;

  private constructor() {}

  static getInstance = () => {
    if (!this.instance) {
      this.instance = new MeriseHelper();
    }
  ...
  addItem = <T extends Entity | Association | Relation>(props: MeriseAddItemProps<T>): MeriseResult<T, null> => {
    ...
    if (itemType === MeriseItemTypeEnum.RELATION) {
      ...
      const validation = this.validateRelationCreation(source, target, relations);
  ...
  findItemById = <T extends Entity | Association | Relation>(
